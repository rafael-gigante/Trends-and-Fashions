...
"""
    interaction(i::Int, j::Int, N_i::Vector{Vector{Int}}, L_i::Vector{Int}, N::Int)

Define the interaction between agents `i` and `j` in the system.
"""
function interaction(i::Int, j::Int, N_i::Vector{Vector{Int}}, L_i::Vector{Int}, N::Int)
    if N_i[2][L_i[j]] > N_i[2][L_i[i]]
        q = (N_i[2][L_i[j]] - N_i[2][L_i[i]]) / (N - 2)
        if rand() <= q
            N_i[2][L_i[i]] -= 1
            N_i[2][L_i[j]] += 1
            L_i[i] = L_i[j]
...
"""
function measures(N_i::Vector{Vector{Int}}, N::Int, L::Int)
    Pc = maximum(N_i[2])
    Sc = 0.0
    S = 0.0
    for i in 1:L
