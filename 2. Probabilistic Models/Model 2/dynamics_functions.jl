...
"""
    interaction(i::Int, j::Int, N_i::Vector{Vector{Int}}, L_i::Vector{Int}, p_i::Vector{Int}, N::Int)

Define the interaction between agents `i` and `j` in the system.
"""
function interaction(i::Int, j::Int, N_i::Vector{Vector{Int}}, L_i::Vector{Int}, p_i::Vector{Int}, N::Int)
    if p_i[L_i[j]] > p_i[L_i[i]]
        q = (p_i[L_i[j]] - p_i[L_i[i]]) / (2 * N)
        if rand() <= q
            N_i[2][L_i[i]] -= 1
            N_i[2][L_i[j]] += 1
            L_i[i] = L_i[j]
        end
    elseif p_i[L_i[j]] == p_i[L_i[i]]
        N_i[2][L_i[i]] -= 1
        N_i[2][L_i[j]] += 1
        L_i[i] = L_i[j]
    end
